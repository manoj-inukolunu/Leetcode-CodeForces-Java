...
public class B1003 {
  ...
  public static void main(String args[]) {
    ...
    try (PrintWriter out = new PrintWriter(System.out)) {
      ...
      if (even && buffer.toString().startsWith("00")) {
        buffer.deleteCharAt(0);
        buffer.append('0');
      } else if (even && buffer.toString().startsWith("11")) {
        buffer.deleteCharAt(0);
        buffer.append('1');
      }
      out.println(buffer);
      ...
      for (int i = 0; i < buffer.length(); i++) {
        ...
        if (i + 1 < buffer.length() && buffer.charAt(i) != buffer.charAt(i + 1)) {
          count++;
        }
      }
      assert gA == 0;
