    ...
    try (PrintWriter out = new PrintWriter(System.out)) {
      ...
      while (t-- > 0) {
        ...
        for (int i = 1; i < arr.length; i++) {
          //incl as positive
          if (!neg.isEmpty()) {
            Long maxNeg = neg.last();
            if (maxNeg != null) {
              pos.add(maxNeg + arr[i]);
            }
          } else {
            pos.add(arr[i]);
          }
          //incl as negative
          Long maxPos = pos.last();
          neg.add(maxPos - arr[i]);
        }
        if (!neg.isEmpty() && !pos.isEmpty()) {
          out.println(Math.max(neg.last(), pos.last()));
        } else if (neg.isEmpty() && !pos.isEmpty()) {
          out.println(pos.last());
        } else if (!neg.isEmpty()) {
          out.println(neg.last());
        }
      }
