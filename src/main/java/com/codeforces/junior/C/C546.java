...
public class C546 {
  ...
  public static void main(String args[]) {
    ...
    try (PrintWriter out = new PrintWriter(System.out)) {
      ...
      long[] a = scanner.readArray(k1);
      int k2 = scanner.nextInt();
      long[] b = scanner.readArray(k2);
      HashSet<String> seen = new HashSet<>();
      LinkedList<Long> aL = new LinkedList<>();
      for (long num : a) {
        aL.add(num);
      }
      LinkedList<Long> bL = new LinkedList<>();
      for (long num : b) {
        bL.add(num);
      }
      int turn = 0;
      while (true) {
        ...
        String state = getState(aL, bL);
        if (seen.contains(state)) {
          out.println(-1);
          return;
        }
        seen.add(state);
        Long first = aL.pollFirst();
        Long second = bL.pollFirst();
        if (first > second) {
          aL.addLast(second);
          aL.addLast(first);
        } else {
  ...
  private static String getState(LinkedList<Long> aL, LinkedList<Long> bL) {
