    ...
    try (PrintWriter out = new PrintWriter(System.out)) {
      ...
      for (int i = 0; i < s.length(); i++) {
        ...
        if (Character.isUpperCase(ch) && map.containsKey(Character.toLowerCase(ch))) {
          cW++;
          map.put(Character.toLowerCase(ch), map.get(Character.toLowerCase(ch)) - 1);
          if (map.get(Character.toLowerCase(ch)) <= 0) {
            map.remove(Character.toLowerCase(ch));
          }
        } else if (Character.isLowerCase(ch) && map.containsKey(Character.toUpperCase(ch))) {
          cW++;
          map.put(Character.toUpperCase(ch), map.get(Character.toUpperCase(ch)) - 1);
          if (map.get(Character.toUpperCase(ch)) <= 0) {
