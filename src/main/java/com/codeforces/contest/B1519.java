    ...
    try (PrintWriter out = new PrintWriter(System.out)) {
      ...
      while (t-- > 0) {
        ...
        while (!queue.isEmpty()) {
          ...
          if (curr.first == n && curr.second == m && curr.third == k) {
            out.println("YES");
            continue top;
          }
          if (!visited.contains(curr)) {
            visited.add(curr);
            if (curr.third + curr.first <= k) {
              queue.add(new Pair(curr.first, curr.second + 1, curr.third + curr.first));
            }
            if (curr.third + curr.second <= k) {
              queue.add(new Pair(curr.first + 1, curr.second, curr.third + curr.second));
            }
